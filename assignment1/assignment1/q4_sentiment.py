...
def getSentenceFeatures(tokens, wordVectors, sentence):
    ...
    ### YOUR CODE HERE
    word_index_in_sen = [ tokens[w] for w in sentence ] #获取每个单词的索引
    word_vecotr_of_sec = wordVectors[word_index_in_sen] #根据索引获得向量
    sentVector = np.mean(word_vecotr_of_sec, axis=0)
    ### END YOUR CODE
...
def getRegularizationValues():
    ...
    ### YOUR CODE HERE
    values = np.logspace(-5,5,11)  #起始是10^-5, 结束是10^5，产生一个长度为11的等比数列,base 是10
...
def main(args):
    ...
    for i in xrange(nTrain):
        ...
        print "words", words
        print "trainFeatures", i, trainFeatures[i][0]

    print "dimVectors", dimVectors
    print "nTrain", nTrain
    ...
    for reg in regValues:
        ...
        clf = LogisticRegression(C=1.0/(reg + 1e-12)) #C是正则项的倒数
