...
#x是shape应该是（n_samples, n_class)吧
def softmax(x):
    """
    ...
    print "max_x", max_x
    x_subtract_max=tf.subtract( x, tf.reshape(max_x, [-1,1]))

    exp_x = tf.exp(x_subtract_max)
    print "exp_x.shape", exp_x
    sum_exp_x = tf.reduce_sum(exp_x, axis=1)

    out=exp_x/tf.reshape(sum_exp_x, [-1,1])
    ### END YOUR CODE
...
def cross_entropy_loss(y, yhat):
    ...
    ### YOUR CODE HERE
    yhat = tf.log(yhat)
    loss_matrix = tf.multiply(tf.to_float(y), yhat)

    # out = -tf.reduce_sum( loss_matrix )
    out = tf.negative(tf.reduce_sum( loss_matrix ))
...
def test_softmax_basic():
    ...
    """
    testData = tf.constant(np.array([[1004, 1002], [3, 4]]), dtype=tf.float32)
    test1 = softmax(tf.constant(np.array([[1004, 1002], [3, 4]]), dtype=tf.float32))
    print test1.shape
    max_x = tf.reduce_max(testData, axis=1)
    xx=tf.subtract( testData, tf.reshape(max_x, [-1,1]))
