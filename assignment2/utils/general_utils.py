...
def get_minibatches(data, minibatch_size, shuffle=True):
    ...
    """
    list_data = type(data) is list and (type(data[0]) is list or type(data[0]) is np.ndarray) #判断data的元素是不是还是list
    data_size = len(data[0]) if list_data else len(data)
    indices = np.arange(data_size)
    if shuffle:
        np.random.shuffle(indices)
    for minibatch_start in np.arange(0, data_size, minibatch_size):#arange用来使用一定的步长生成区间内的序列
        minibatch_indices = indices[minibatch_start:minibatch_start + minibatch_size]
        yield [minibatch(d, minibatch_indices) for d in data] if list_data \
            else minibatch(data, minibatch_indices)

#这个实现应该是因为numpy中的ndarray和python中的list的索引方式有些不同
