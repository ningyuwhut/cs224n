...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE
        # W=tf.Variable()
        self.W = tf.get_variable("W",  dtype=tf.float32,shape=(self.config.n_classes, self.config.n_features),
                                          initializer=tf.zeros_initializer)
        self.b = tf.get_variable("b", dtype=tf.float32, shape=(self.config.n_classes),initializer=tf.zeros_initializer)

        fx = tf.matmul(self.input_placeholder, tf.transpose(self.W))+tf.reshape(self.b, [-1,self.config.n_classes])

        print "fx.shape", tf.shape
        pred = softmax(fx)
        ### END YOUR CODE
        return pred
