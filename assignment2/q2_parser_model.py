...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        input_embedding=tf.nn.embedding_lookup(embedding, self.input_placeholder)
  #      print "input_embedding.type", type( input_embedding)
  #      print  "input_embedding.shape", input_embedding.shape
        print input_embedding.shape[1]

        input_embedding_shape_list = input_embedding.get_shape().as_list() #.shape[1]*input_embedding.shape[2]
  #      print "input_embedding_shape_list", input_embedding_shape_list
     #   print type(concate_len)

 #       embeddings = tf.reshape(input_embedding,[-1, input_embedding.shape[1]*input_embedding.shape[2]])

        embeddings = tf.reshape(input_embedding,[-1, input_embedding_shape_list[1]* input_embedding_shape_list[2]])
        #reshap的第2个参数是shape函数，列表的第一个值不能是None，为啥
        #上面的实现会报异常，原因还不太清楚
    ...
    def add_prediction_op(self):
        ...
        feature_num = tf.shape(x)[1]
    #    embedding_size = tf.shape(x)[2]
  #      print "feature_num", feature_num
  #      print "feature_num.type", type(feature_num)
   ...
   #     print "embedding_size", embedding_size
        ...
        #第一次实现时W、U、b1、b2都没有定义成Variable,在运行的时候loss一直为常数，为啥

        h=tf.nn.relu(tf.matmul(x,W)+b1)
        h_drop = tf.nn.dropout(h, self.config.dropout) #[None, hidden_size]
        print "h_drop", type(h_drop)
 #       h_drop = tf.nn.dropout(h, self.dropout_placeholder) #[None, hidden_size]
 #       print "h_drop", type(h_drop)
    ...
    def add_loss_op(self, pred):
        ...
        """
        ### YOUR CODE HERE
        total_loss = tf.nn.softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=pred) #这个函数返回的tensor长度和batch_size 相同
    ...
    def fit(self, sess, saver, parser, train_examples, dev_set):
        best_dev_UAS = 0# 记录验证集（evaluation set or dev set) 上的最佳指标
        for epoch in range(self.config.n_epochs): #每个epoch
