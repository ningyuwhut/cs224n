...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        #这个函数的实现学习到了python list extend和append的区别
        new_sentence = []
        new_labels = []
        mask = []
        if len( sentence) < max_length:
            new_sentence.extend(sentence + [zero_vector] * (max_length - len(sentence)))
            new_labels.extend( labels + [zero_label] * (max_length-len(sentence)))
            mask = [ True if i <  len( sentence) else False for i in range(max_length) ]
        else:
            new_sentence.extend(sentence[0:max_length])
            new_labels.extend( labels[0:max_length])
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        embeddings_var  = tf.Variable(self.pretrained_embeddings) #如何使用get_variable 并且用numpy array来初始化
    #    x = tf.get_variable( shape=[2, 4], initializer=init)
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE (~4-6 lines)
        self.U = tf.get_variable(name = "U", shape = (Config.hidden_size, Config.n_classes), dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())
        self.b = tf.get_variable(name = "b", shape = (Config.n_classes), dtype=tf.float32, initializer=tf.zeros_initializer() )

        h = tf.zeros(shape =(tf.shape(self.input_placeholder)[0],Config.hidden_size ), dtype=tf.float32)
        #这里先初始化成(batch_size,hidden_size)
...
#这两行代码没问题
               # transposed_x = tf.transpose(x, perm=(1,0,2))

               # x_t = tf.gather(transposed_x, tf.constant(time_step))

               # o_t, h_t = cell(x_t, h)
                o_t, h_t = cell(x[:,time_step,:], h)

                o_drop_t = tf.nn.dropout(o_t, keep_prob=self.dropout_placeholder)

                y_t = tf.matmul(o_drop_t, self.U ) + self.b
        ...
        print "shape", preds[0].shape

        preds = tf.stack(preds, axis=1)
        print "shape", preds[0].shape
        #preds的维度从长度为max_len的[ batch_size, n_classes) 变成了 [ batch_size, max_len , n_classes ]



        #axis = 0 和 1 的区别
        #下面这段代码有问题
        #用325行代码取代下面的代码之后测试结果变得正常
        #preds = tf.stack(preds, axis=0)
        # print "preds.shape", preds.shape
        # new_preds = tf.reshape(preds,shape = (-1, self.max_length, Config.n_classes))
        # print "preds.shape", tf.shape(new_preds), new_preds.shape
        # #为什么tf.shape()和new_grads.shape的结果不同
        # preds = new_preds
        ### END YOUR CODE
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        print "preds", tf.shape(preds)
        print "mask", self.mask_placeholder.shape
       # masked_preds = tf.boolean_mask(preds, self.mask_placeholder)
     #   print "masked_preds.shape", tf.shape(masked_preds),masked_preds.shape
        print "labels.shape", tf.shape(self.labels_placeholder),self.labels_placeholder.shape
        ...
        masked_loss = tf.boolean_mask(total_loss, self.mask_placeholder)
