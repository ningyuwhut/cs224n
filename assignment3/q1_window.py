...
class Config:
    ...
    #注意window_size下window的大小是2*window_size +1
    ### END YOUR CODE
    n_classes = 5
    dropout = 0.5
    embed_size = 50
    ...
    ### YOUR CODE HERE (5-20 lines)
        for i, word in enumerate(sentence):
          #  print i ,word
            ...
            padding_end = []
            if start_index < 0 :
                padding_start += (-start_index ) * start

                start_index = 0

            if  end_index > (len( sentence)-1) :
                padding_end += ( end_index - (len(sentence)-1)  ) * end
                end_index = len(sentence)-1

           # print "start_index", start_index
           #print "end_index", end_index

            # print "padding_start", padding_start
            # print "padding_end", padding_end
            # print "mid", list(chain.from_iterable(sentence[start_index: end_index+1])), sum( sentence[start_index: end_index+1], [])

            #注意sum的用法，第2个参数是start，表示初始结果
            word_features = (padding_start + sum(sentence[start_index: end_index+1], []) + padding_end)
            #print "word_features", word_features
            data_points = ( word_features, labels[i])

            windowed_data.append(data_points)
...
class WindowModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.labels_placeholder = tf.placeholder( tf.int32, shape = (None,)) #这里labels_placeholder需要是one hot 形式的 ,错 self.config.n_classes
    ...
    def add_embedding(self):
        ...
        input_embedding = tf.nn.embedding_lookup( whole_embeddings, self.input_placeholder)
        embeddings = tf.reshape( input_embedding, shape=(-1, self.config.n_window_features*self.config.embed_size))
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        self.W = tf.get_variable( name="W", shape=(self.config.n_window_features*self.config.embed_size, self.config.hidden_size), dtype=tf.float32,
                                 initializer=tf.contrib.layers.xavier_initializer())

        self.b1 = tf.Variable(tf.zeros(shape=(self.config.hidden_size), dtype=tf.float32))

        h = tf.nn.relu( tf.add( tf.matmul(x, self.W), self.b1 ))

        h_drop = tf.nn.dropout(h, dropout_rate)

        self.U = tf.get_variable( name="U", shape=(self.config.hidden_size, self.config.n_classes), dtype = tf.float32,
                                 initializer=tf.contrib.layers.xavier_initializer())

        self.b2 = tf.Variable(tf.zeros(shape=(self.config.n_classes), dtype=tf.float32))

        pred = tf.add( tf.matmul( h_drop, self.U) , self.b2)
    ...
    def add_loss_op(self, pred):
        ...
        #因为这里的预处理得到的label的batch不是one hot形式的，所以需要使用 sparse_softmax_cross_entropy_with_logits
        ### YOUR CODE HERE (~2-5 lines)
        sum_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits = pred )
    ...
    def train_on_batch(self, sess, inputs_batch, labels_batch):
        ...
        print "intpu_batch.shape", inputs_batch.shape
