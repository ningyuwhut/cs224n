...
class SequencePredictor(Model):
    ...
    def add_prediction_op(self): 
        ...
        outputs, last_states = tf.nn.dynamic_rnn(  cell, inputs = x, dtype=tf.float32) #, sequence_length=self.config.max_length,

        #虽然用的dynamic rnn，但是输入序列的长度还都是一样的????
        preds = tf.sigmoid(last_states)
        ### END YOUR CODE
    ...
    def add_loss_op(self, preds):
        ...
        l2_loss =tf.nn.l2_loss(preds-y) #l2_loss的维度和preds的维度相同
    ...
    def add_training_op(self, loss):
        ...
        # grads_and_vars = optimizer.compute_gradients(loss) #返回的是元素是元组(grads, var)的list
        # grads = [ ele[0] for ele in grads_and_vars ]

        gradients, vriables = zip(*optimizer.compute_gradients(loss))
        # compute_gradients 返回元素是元组(grads, var)的list，*运算符将list解压成一个个元组，zip函数将这些元组的对应元素组成一个新的元组，最后返回由这些新的元组组成的list
        #具体来说，*运算符之后就变成了(grad1, var1),(grad2,var2),...,(gradn,varn),然后zip函数把这些元组中的第一个元素放在一块做成一个新的元组，第2个元素做成一个新的元组，即所有的grad是yige
        #元组，所有的var是一个元组，所以最后就成了[(grad1,grad2,...,gradn), (var1, var2,...,varn)]
        ...
        if self.config.clip_gradients:
          #  capped_grads_and_vars = [(tf.clip_by_norm(gv[0], self.config.max_grad_norm), gv[1])   for gv in grads_and_vars]
          # self.grad_norm = tf.global_norm(capped_grads_and_vars[0])
          # train_op = optimizer.apply_gradients(capped_grads_and_vars)
            clipped_gradients, _ = tf.clip_by_global_norm(gradients, self.config.max_grad_norm)
            gradients = clipped_gradients
         # clipped_gradients, self.grad_norm = tf.clip_by_global_norm(grads, self.config.max_grad_norm)

        self.grad_norm = tf.global_norm( gradients)

        train_op = optimizer.apply_gradients(zip(gradients, vriables))
